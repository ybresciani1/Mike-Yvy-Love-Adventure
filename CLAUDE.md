...
src/scenes/              35 scenes + index.js (the registry)
```
...
- **The controls are a keyboard.** The D-pad and the `A`/`B` buttons dispatch
  ...
  `cursors.left.isDown` and `JustDown(this.spaceKey)` in the 35 scenes, and the
...
prompt up on touch while keeping prompts in proportion to each other. Twelve
...
`WolvesBarScene`, `FlowerFieldsScene` — and all of
