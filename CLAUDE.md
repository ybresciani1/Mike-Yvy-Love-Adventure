...
src/scenes/              36 scenes + index.js (the registry)
```
...
- **The controls are a keyboard.** The D-pad and the `A`/`B` buttons dispatch
  ...
  `cursors.left.isDown` and `JustDown(this.spaceKey)` in the 36 scenes, and the
...
prompt up on touch while keeping prompts in proportion to each other. Thirteen
...
`WolvesBarScene`, `FlowerFieldsScene`, `CafeSecretScene` — and all of
